        ...
        if ($Eleve > 0) {
            ...
            if ($DateNaissance > $aujourd_hui) {
                ...
                if ($age < 3) {
                    ...
                    $req = $connexion->prepare("INSERT INTO `eleve`(nom, postnom, prenom, genre, adresse, lieuNaissance, dateNaissance, nomPere, nomMere, numeroParent, photo, statut) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)");
                    $resultat = $req->execute([$nom, $postnom, $prenom, $genre, $Adresse, $LieuNaissance, $DateNaissance, $NomPere, $NomMere, $phone, $photo, $statut]);
                    if ($resultat == true) {
